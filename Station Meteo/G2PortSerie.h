#ifndef G2PortSerie_H
#define G2PortSerie_H
...
public:
    ...
    */
    ...
    /*!
      ...
      \sa fermer()
    */
    int Ouvrir(const char * nomDuPortSerie);

	bool ModifierVitesse(G2PortSerieVitesse vitesse);

	bool ModifierLongueurDonnees(G2PortSerieLongueurDonnees longueurDonnees);

	bool ModifierNombreBitStop(G2PortSerieBitStop bitStop);

	bool ModifierParite(G2PortSerieParite parite);
	...
	*/
	bool EnvoyerDesOctets(const char trame[], int nbOctets) const;
	...
	*/
	int LireDesOctets(char message[], int tailleMaximaleDuMessage) const;
